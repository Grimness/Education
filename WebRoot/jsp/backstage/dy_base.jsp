...
.btn.disabled{cursor:not-allowed;background-image:none;opacity:.65;filter:alpha(opacity=65);box-shadow:none; pointer-events:none;} 
 ...
 var FName=document.getElementById('FName'+change).value;
	...
	//alert(FNumber.value);
	//alert(fid.value);
	 window.location.href="${pageContext.request.contextPath }/changedy.do?fid="+fid+"&FNumber="+FNumber+"&FName="+encodeURIComponent(FName); 
 }
 </script>	
</head> 
<body> 
...
<tr > 
<td><input type="button" value="新增" class="btn btn-primary radius" onclick="javascript:window.location.href='add_dy.do'"></td><td colspan="3"> </td>
</tr>
<tr> 
<th>序号</th> 
<th>单元编码</th> 
...
<td><%=i %></td>
<td  style="display:none"><input type="text" name="fid<%=i %>" id="fid<%=i %>" value="${dy.fid}"></td>
<td><input type="text" name="FNumber<%=i %>" id="FNumber<%=i %>" value="${dy.FNumber}"></td>
<td><input type="text" name="FName<%=i %>" id="FName<%=i %>" value="${dy.FName}"></td>
<td><input class="btn btn-success radius" type="button" value="修改" onclick="change(<%=i %>)">&nbsp;<input class="btn btn-danger radius" type="button" value="删除" onclick="javascript:window.location.href='deldy.do?fid=${dy.fid}'"></td> 
</tr> 
<%i++; %>
</c:forEach>
