...
<body> 
<form id="form1" name="form1" method="post" action="search_teachers.do">
<table>
<tr>
...
<th></th> 
 ...
 <tbody id="table2">
 <c:forEach items="${getteacher}" var="t">
<tr > 
<td>${t.fname}</td> 
<td><c:if test="${t.fsex==0}" >女</c:if><c:if test="${t.fsex==1}" >男</c:if></td> 
<td>${t.fphone}</td> 
<td>${t.areaname}</td> 
<td>${t.schoolname}</td> 
<td>${t.fsubjectid}</td> 
<td><c:if test="${t.fright==0}" >正常</c:if><c:if test="${t.fright==1}" ><font color=red>禁用</font></c:if></td> 
<td><input class="btn btn-success radius" type="button" value="启用" onclick="javascript:window.location.href='updateteacher_right.do?fright=0&fid=${t.fid}'">&nbsp;<input class="btn btn-danger radius" type="button" value="禁用" onclick="javascript:window.location.href='updateteacher_right.do?fright=1&fid=${t.fid}'"></td> 
</tr> 
</c:forEach>
</tbody>
</table> 
