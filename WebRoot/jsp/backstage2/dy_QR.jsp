...
${message}
	...
	function add(){
		...
		var unit = $("#funit").find("option:selected").text();//单元名称
		var unitdesc = $("#funitdesc").val();//单元描述
		var unitcode = version+subject+grade+book+unit;
		window.location.href="add_dycode.do?fname="+unitcode+"&funitname="+unitdesc;
	}
	
	function update(id,fid){
		if(id<10){
			id = "0"+id;
		}
		...
		var unitdesc = $(".funit"+fid+id).val();//单元描述
		var unitcode = version+subject+grade+book+id;
		window.location.href="update_dycode.do?fname="+unitcode+"&fid="+fid+"&funitname="+encodeURI(unitdesc);
	}
	
	function deletedy(id,fid){
		if(id<10){
			id = "0"+id;
		}
		var version = $("#fversionid"+fid).attr("name");//版本
		...
		var book = $("#FVolume"+fid).attr("name");//上下册
		var unitcode = version+subject+grade+book+id;
		window.location.href="delete_dycode.do?fname="+unitcode+"&fid="+fid;
	}
...
<body>
	<div style="border: 0px solid #D1D1D1; height: 100%">
		...
		<div style="border: 0px solid #D1D1D1;margin-top: 10px">
			...
			科目： <select id="fsubjectid" name="fsubjectid"
			...
			</select> &nbsp; 年级： <select id="fclassid" name="fclassid">
				<option value="3">三年级</option>
				<option value="4">四年级</option>
				<option value="5">五年级</option>
				<option value="6">六年级</option>
			</select> &nbsp; 上下册： <select id="FVolume" name="FVolume">
				...
				<c:forEach items="${dy}" var="dy">
					<option value="${dy.FNumber}">${dy.FNumber}</option>
				</c:forEach>
				</select>
				
			
		</div>
		单元描述名称：<input type="text"  id="funitdesc" >
		<input type="button"  value="新增单元描述" class="btn btn-primary radius"
				onclick="add()">
		...
		<div style="margin-top: 10px">
			<table width="100%" class="table">
				...
				<tr align="center">
					<td>${qc.ftitle}</td>
					<td><%-- <select  id="fsubjectid${qc.fid}" name="fsubjectid${qc.fid}"
						onchange="setSecond2(${qc.fid})">
							<option  value="${qc.fsubject}">${qc.fsubjectname}</option>
							<option value="YW">语文</option>
							<option value="SX">数学</option>
							<option value="YY">英语</option>
					</select> --%>
					<span id="fsubjectid${qc.fid}" name="${qc.fsubject}">${qc.fsubjectname}</span></td>
					<td><%-- <select  id="fversionid${qc.fid}" name="fversionid${qc.fid}">
						<option value="${qc.fversion}">${qc.fversionname}</option>
					</select> --%>
					<span id="fversionid${qc.fid}" name="${qc.fversion}">${qc.fversionname}</span></td>
					<td><%-- <select id="fclassid${qc.fid}" name="fclassid${qc.fid}">
							<option value="${qc.fgrade}">${qc.fgradename}</option>
							<option value="3">三年级</option>
							<option value="4">四年级</option>
							<option value="5">五年级</option>
							<option value="6">六年级</option>
					</select> --%><span id="fclassid${qc.fid}" name="${qc.fgrade}">${qc.fgradename}</span></td>
					<td><%-- <select   id="FVolume${qc.fid}" name="FVolume${qc.fid}">
							<option value="${qc.fbook}">${qc.fbookname}</option>
							<option value="2">下册</option>
							<option value="1">上册</option>
							<!-- <option value="0">不分册</option> -->
					</select> --%><span id="FVolume${qc.fid}" name="${qc.fbook}">${qc.fbookname}</span></td>
					<td>
					<input type="text" style="width: 140px" id="${qc.fid}${qc.funit}" class="funit${qc.fid}${qc.funit} " value="${qc.funitname}" ></td>
					<td><input  class="btn btn-success radius" type="button"
						value="修改" onclick="update(${qc.funit},${qc.fid})">&nbsp;<input
						class="btn btn-danger radius" type="button" value="删除" onclick="deletedy(${qc.funit},${qc.fid})"></td>
				</tr>
				</c:forEach>
				</tbody>
			</table>
